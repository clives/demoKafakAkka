...
class TestcontainersUsingDeviceReadingMsgSpec extends SpecBase with TestcontainersKafkaPerClassLike {
  ...
  "plain source" should {
    "not lose any messages when a Kafka node dies" in assertAllStagesStopped {
      ...
      val result = Source(0L to totalMessages)
        ...
        .map(number => new ProducerRecord(topic, partition0, DefaultKey,
          DeviceReading(UUID.randomUUID(), 1.0f, MeasureUnit.Fahrenheit,
